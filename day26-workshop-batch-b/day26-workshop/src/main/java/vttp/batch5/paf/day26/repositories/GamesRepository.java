...
public class GamesRepository {
    ...
    public Optional<JsonObject> findBoardGameByName(String name){
        //we are searching board games by its name
        Criteria criteria = Criteria.where(F_NAME)
            .regex(name, "i");

        Query query = Query.query(criteria)
            .with(Sort.by(Sort.Direction.DESC, F_RANKING))
            .limit(1); //limit our search to only 1, although we could have received multiple
        query.fields()
            .exclude(F_ID)
            .include(F_GID, F_NAME, F_RANKING, F_URL, F_IMAGE);

        //template.find() method usually returns a list/array
        ...
        Document doc = result.get(0); //we limited to 1 in the query above
